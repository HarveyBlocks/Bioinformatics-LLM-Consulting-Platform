window.PagePoints = {
	async render() {
		...
		try {
			const {ok, res} = await API.safe(
					API.points.selfPointHistory, historyLimit, historyPage);
			if (ok && res?.data) {
		...
		const historyRows = pointsHistory.length > 0 ? pointsHistory.map(item => {
			...
			const time = item.createDate || item.time || item.createTime ||
					item.createdAt || item.date || item.timestamp || '';
			const flow = item.flow !== undefined ?
					item.flow :
					(item.points || item.amount || item.change || item.pointChange || 0);
			const reason = item.reason || item.source || item.description ||
					item.type || '';
		...
		<div class="my-4">
			...
			<div class="card">
				...
				`<a class="page-link" href="${makeHistoryLink(Math.max(
						1,
						historyPage - 1,
				))}" data-history-link="${makeHistoryLink(
						Math.max(1, historyPage - 1))}">上一页</a>`}
							</li>
							<li class="page-item active"><span class="page-link">${historyPage}</span></li>
							<li class="page-item ${historyNextDisabled}">
								${historyNextDisabled ?
				'<span class="page-link">下一页</span>' :
				`<a class="page-link" href="${makeHistoryLink(
						historyPage + 1)}" data-history-link="${makeHistoryLink(
						historyPage + 1)}">下一页</a>`}
							</li>
						</ul>
					</nav>
		...
		root.querySelector('#pointsHistoryPager')?.
		...
		root.querySelectorAll('[data-detail-id]').forEach(btn => {
			btn.addEventListener('click', async () => {
				...
				// 处理图片 URL（空字符串也视为无效）
				const pic1 = (detail.pictureUrl1 && detail.pictureUrl1.trim()) ||
						'./assets/img/gift1.svg';
				const pic2 = (detail.pictureUrl2 && detail.pictureUrl2.trim()) ||
						'./assets/img/gift2.svg';
				const pic3 = (detail.pictureUrl3 && detail.pictureUrl3.trim()) ||
						'./assets/img/gift3.svg';
				// 处理积分和库存显示
				const cost = detail.cost !== undefined && detail.cost !== null ?
						detail.cost :
						'--';
				const storage = detail.storage !== undefined && detail.storage !==
				null ? detail.storage : '--';
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<div class="row g-2 mb-2">
								<div class="col-4"><img class="img-fluid rounded lazy" data-src="${pic1}" alt="${detail.title ||
				'礼品图片1'}"></div>
								<div class="col-4"><img class="img-fluid rounded lazy" data-src="${pic2}" alt="${detail.title ||
				'礼品图片2'}"></div>
								<div class="col-4"><img class="img-fluid rounded lazy" data-src="${pic3}" alt="${detail.title ||
				'礼品图片3'}"></div>
							</div>
						...
						<div class="modal-footer">
							...
							<button type="button" class="btn btn-primary" id="detailExchange"${isStorageZero ?
						' disabled' :
						''}>立即兑换</button>
						</div>
					</div>

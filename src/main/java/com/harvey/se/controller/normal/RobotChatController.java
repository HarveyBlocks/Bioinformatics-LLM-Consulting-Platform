...
import com.harvey.se.pojo.dto.ChatMessageFromUserDto;
...
public class RobotChatController {
    ...
    public Result<Long> streamChatQwen(@RequestBody ChatMessageFromUserDto dto) throws InterruptedException {
        return new Result<>(robotChatService.chat(UserHolder.getUser(), dto.getMessage(), 0));
    }

    @PostMapping(value = "/chat/deepseek")
    @ApiOperation(
            "用户问问题, Deepseek进行回答, 回答使用流式回答. 在回答期间, 用户再问问题, 问题将被忽略. 请客户端阻止用户问问题. 有积分拿, 每日五次, 每次五分 ")
    public Result<Long> streamChatDeepseek(@RequestBody ChatMessageFromUserDto dto) throws InterruptedException {
        return new Result<>(robotChatService.chat(UserHolder.getUser(), dto.getMessage(), 1));
    }
    ...
    public Result<List<ChatMessageDto>> queryByTimeRange(
        ...
        return new Result<>(chatMessageService.queryByUser(UserHolder.currentUserId(),
                dateRange,
